...
public class ApiController {
    ...
    public ResponseEntity<?> demoSend(@RequestBody DemoSendRequest req) throws Exception {
        int ttl = Objects.nonNull(req.ttl) ? req.ttl : 5;
        String startDevice = Objects.nonNull(req.startDevice) ? req.startDevice : "phone-alice";
    ...
    public Map<String, Object> meshFlush() {
        ...
        uploads.parallelStream().forEach(up -> {
            ...
            synchronized (results) {
                Long txId = r.transactionId();
                String reason = r.reason();
                results.add(Map.of(
                        "bridgeNode", up.bridgeNodeId(),
                        "packetId", up.packet().getPacketId().substring(0, 8),
                        "outcome", r.outcome(),
                        "reason", Objects.nonNull(reason) ? reason : "",
                        "transactionId", Objects.nonNull(txId) ? txId : -1L
                ));
            }
        });
